...
class CDONScraper:
    ...
    async def crawl_category(self, category_url: str, max_pages: int = 5) -> List[Movie]:
        ...
        for i, url in enumerate(product_urls, 1):
            try:
                ...
                if movie and self.is_bluray_format(movie.title, movie.format):
                    ...
                    logger.debug(f"✗ Skipped: not a Blu-ray or parsing failed")
...
async def main():
    ...
    movies = await scraper.crawl_category(bluray_url, max_pages=1)  # Just 1 page for demo
